...
try {
    ...
    $rows = $db
        ->query('SELECT migration, applied_at FROM schema_migrations ORDER BY migration ASC')
    ...
    echo "WBGL Migration Status" . PHP_EOL;
    ...
    foreach ($fileNames as $name) {
        if (isset($appliedMap[$name])) {
            echo "[APPLIED] {$name} @ {$appliedMap[$name]}" . PHP_EOL;
        } else {
    ...
    echo str_repeat('-', 72) . PHP_EOL;
    echo "Total: " . count($fileNames) . " | Pending: {$pending} | Applied: " . (count($fileNames) - $pending) . PHP_EOL;
    exit(0);
} catch (Throwable $e) {
